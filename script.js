...
const getAllData = function(){
    get(child(dbRef, 'PlayerSet')).then((snapshot)=>{
        playersList = [];
        count = 1;
        document.getElementById("lbbody").innerHTML = '<tr><th colspan="3" id="lbheading">LEADERBOARD <i class="fa-solid fa-sort-down" style="color:green;"></th></tr><tr><th>Position</th><th>Name</th><th>Score</th></tr>'
...
function saveUsername(){
    get(child(dbRef, 'PlayerSet')).then((snapshot)=>{
        ...
        try {
            playersList.forEach(player => {
                if (document.getElementById("username").value.toLowerCase() == player.username.name.toLowerCase()){
...
function start(){
    ...
    countdownInterval = setInterval(()=>{
        ...
        if (second == 0){
                ...
                console.log("kuchh toh gadbad hai daya")
                clearInterval(countdownInterval)
                if (difficulty == "easy"){
...
function updateLB(){
    document.getElementById("lbheading").addEventListener('click', ()=>{
        if (lbHidden){
            ...
            lbHidden = false;
            let lb = Array.from(document.getElementById("leaderboard").childNodes[1].children)
            lb.splice(0,1)
